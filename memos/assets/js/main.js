...
document.addEventListener('click', function (event) {
	...
	if (target.tagName.toLowerCase() === 'a' && target.getAttribute('href').startsWith('#')) {
		...
		var tagresult = `<span class='tag-span'><a rel='noopener noreferrer' href=''><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon-text"><path d="M12 2H2v10l9.29 9.29c.94.94 2.48.94 3.42 0l6.58-6.58c.94-.94.94-2.48 0-3.42L12 2Z"></path><path d="M7 7h.01"></path></svg>${tag}</a></span>`
...
// 已内置样式，修改 API 即可使用
function fetchDB() {
    var dbAPI = "https://api.xzgljiang.com/douban/";  // 修改为自己的 API
...
document.addEventListener("DOMContentLoaded", () => {
    let limit = 8
    var memoUrl = memo.host          // memos url
            ...
            if(data[i].resourceList && data[i].resourceList.length > 0){
              ...
              for(var j=0;j < resourceList.length;j++){
                ...
                if(resexlink){
                  ...
                  resLink = memoUrl+'o/r/'+resourceList[j].id+'/'+resourceList[j].filename
                }
                if(restype == 'image' && nowNum <= limit ){
                    nowNum ++
                    result += '<div class="memos-photo"><img class="photo-img" loading="lazy" decoding="async" src="'+resLink+'"/><span class="photo-title"><a href="' + memo.host + 'm/' + data[i].id + '" target="_blank">'+galleryTitle+'</a></span><span class="photo-time">'+galleryTime+'</span></div>'
                }
              }
            }
        }
